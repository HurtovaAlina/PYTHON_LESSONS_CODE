...
def add_contact(phone_book: Dict[str, Dict[str, str]]):
    ...
    contact_mail = input("Enter contact mail ")

    if contact_name not in phone_book:
        phone_book[contact_name] = {
            "phone": contact_phone,
            "email": contact_mail
        }

def save_to_json(phone_book: Dict[str, Dict[str, str]], filename: str = "phone_book.json"):
    with open(filename, "w", encoding="utf-8") as file:
        json.dump(phone_book, file, indent=4, ensure_ascii=False)

def load_json(filename:str = "phone_book.json") -> Dict[str, Dict[str, str]]:
    with open(filename, "r", encoding = "utf-8") as file:
        return json.load(file)
...
loaded_phone_book = load_json()
print(f"Loaded from json {loaded_phone_book}")

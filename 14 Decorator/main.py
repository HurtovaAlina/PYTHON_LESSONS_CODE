...
base_finance_report = [
    {
        "article": "income",
        "total_value": "income_value",
        "sub_articles": ["main_income", "other_income"],
    },
    {
        "article": "expences",
        "total_value": "expences_value",
        "sub_articles": ["main_expences", "salary", "other"],
    },
    {
        "article": "profit",
        "total_value": "profit_value",
        "sub_articles": ["main_profit", "other_profit"],
    },
    {
        "article": "taxes",
        "total_value": "taxes_value",
        "sub_articles": ["vat", "salary_tax", "profit_tax", "other"],
    },
    {
        "article": "other expences",
        "total_value": "other_expences_value",
        "sub_articles": ["extraordinary", "financial"],
    },
]
...
def getReportForType(base_report, type):
    ...
    for item in base_report:
        ...
        elif type == "government statistic" and (
            item["article"] == "income"
            or item["article"] == "expences"
...
def setReportDecoratorWrapper(
    report_type,
):  # передаємо тип звітності у декоратор як параметр
    def typeOfReportDecorator(myFunction):
        ...
        print(f"This is report for {report_type}")
...
report_type = input(f"Enter type of report: {report} ")
...
def audit(action_type, user):
    def actionDecorator(myFunction):
        def performAction(*args, **kwargs):
            ...
            log = {"user": user, "action": action_type, "time": time, "params": action}
...
action_type = input(f"Select type of action: {actions} ")
    ...
    def call_count(fn):
        def wrapper(*args):
            ...
            if len(calls) >= max_calls:
                print("Rate limit exceeded. Function was not called. Try later.")
                return None
            calls.append(start)
            return fn(*args)

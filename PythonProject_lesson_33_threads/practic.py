...
# /Users/ahurt/Documents/DOCUMENTS/AI COURSE/PHYTON_LESSONS_CODE/PythonProject_lesson_33_threads/text.txt
path_to_file = input("Enter path ")

word_to_find = input("Enter word ")

def find_word(word_to_find, res:dict[str, int], count_words:dict[str, int]):
    with open(path_to_file, "r") as file:
        text = file.read()

        count = text.count(word_to_find)
        found = count > 0
    res["Word found"] = found
    count_words["Count"] = count

res: dict[str, int] = {}
count_words: dict[str, int] = {}

thread_find_word = threading.Thread(
    target = find_word,
    args = (word_to_find, res, count_words),
)

thread_find_word.start()
thread_find_word.join()
